...
sub hook_load {
    ...
    our $fetch_page_by_hash = $DB->prepare("SELECT id, parent_id, title, ctime, mtime, url_hash FROM ${module_db_prefix}pages WHERE url_hash = ? LIMIT 1");
    our $fetch_page_by_id   = $DB->prepare("SELECT id, parent_id, title, ctime, mtime, url_hash FROM ${module_db_prefix}pages WHERE id = ? LIMIT 1");
...
sub view_page {
    ...
    my $id      = shift;
    my %options = @_;
    my ($parent, $title, $ctime, $mtime, $url_hash);
    ...
    if ($id) {
        ...
        ($id, $parent, $title, $ctime, $mtime, $url_hash) = @{$fetch_page_by_id->fetchrow_arrayref()};
        ...
        # extract data from the query
        ($id, $parent, $title, $ctime, $mtime, $url_hash) = @{$fetch_page_by_hash->fetchrow_arrayref()};
...
sub edit_page {
    ...
    if ($ENV{'REQUEST_METHOD'} eq 'POST') {
        ...
        $has_validated = try {
            ...
            # validate navigation title
            throw 'validation_error', 'A title is required for the site navigation.' unless length $INPUT{'nav_title'};
            
            # validate slug
            throw 'validation_error', 'A slug is required for the URL.' unless length $INPUT{'slug'};
        ...
        if ($has_validated and $INPUT{'save'}) {
            ...
            if ($url ne $page->{'url'}) {
                ...
                $update_url{'title'} = $nav_title;   # update url title
            }
            ...
            # update the page
            my $query = $DB->query(
                "UPDATE ${module_db_prefix}pages SET title = ?, mtime = NOW(), author_id = ?, url_hash = ?, nav_title = ?, slug = ? WHERE id = ?",
                $title, $user::data{'id'}, hash::fast($url), $nav_title, $slug, $page_id
            );
        ...
        # the user is not trying to submit, only preview, or they tried to submit and their page did not validate
            ...
            if ($has_validated) {
                print "\t<content action=\"view\" preview=\"true\" title=\"$title\" nav_title=\"$nav_title\" slug=\"$slug\" parent_url=\"$parent->{url}\">\n";
                for my $field (@fields) {
            ...
            print "\t<content action=\"edit\" id=\"$page_id\" parent=\"$page->{parent_id}\" has_validated=\"$has_validated\" title=\"$title\" nav_title=\"$nav_title\" slug=\"$slug\" parent_url=\"$parent->{url}\" show_nav_link=\"" . ( $show_nav_link ? 'true' : 'false' ) . "\" can_add_files=\"" . ( $PERMISSIONS{'file_add'} ? 'true' : 'false' ) . "\">\n";
            if ($fields_validated) {
...
sub create_page {
    ...
    my $parent = {'id' => 0, 'url' => ''};
    if ($INPUT{'parent'}) {
        $parent = url::get_by_id($INPUT{'parent'});     # _parent($INPUT{'parent'});
    ...
    if ($ENV{'REQUEST_METHOD'} eq 'POST') {
        ...
        if ($has_validated and $INPUT{'save'}) {
            ...
            my $query = $DB->query(
                "INSERT INTO ${module_db_prefix}pages (id, parent_id, title, ctime, mtime, author_id, url_hash, nav_title, slug) VALUES (?, ?, ?, NOW(), NOW(), ?, ?, ?, ?)",
                $url_id, $parent->{'id'}, $title, $USER{'id'}, hash::fast($url), $nav_title, $slug
            );
        ...
        # the user is not trying to submit, only preview, or they tried to submit and their page did not validate
            ...
            # TODO: does not obey inside_content_node
            if ($has_validated) {
                print "\t<content action=\"view\" preview=\"true\" title=\"$title\" parent_url=\"$parent->{url}\" nav_title=\"$nav_title\" slug=\"$slug\">\n";
                for my $field (@fields) {
                    replace_call_data($field->{'translated_value'}, $field->{'call_data'}) if $field->{'call_data'};
                    print "\t\t<$field->{id}>$field->{translated_value}</$field->{id}>\n";
                }
            ...
            # make changes/save form
            style::include_template('create_edit');
            print "\t<content action=\"create\" parent=\"$parent->{id}\" has_validated=\"$has_validated\" title=\"$title\" parent_url=\"$parent->{url}\" nav_title=\"$nav_title\" slug=\"$slug\" show_nav_link=\"" . ( $show_nav_link ? 'true' : 'false' ) . "\" can_add_files=\"" . ( $PERMISSIONS{'file_add'} ? 'true' : 'false' ) . "\">\n";
    ...
    # print a blank create page form or ask for a template
        ...
        if ($template_id) {
            print "\t<content action=\"create\" parent=\"$parent->{id}\" parent_url=\"$parent->{url}\" can_add_files=\"$can_add_files\">\n";
            #print "\t\t<field type=\"text\" />\n";
...
# gets the url parent id from page parent id
sub _parent {
    my $page_id = shift;
    $fetch_page_by_id->execute($page_id);
    return $fetch_page_by_id->fetchrow_arrayref()->[1];
}
...
sub _contextual_admin_menu {
    ...
    if ($PERMISSIONS{'content_admin'}) {
        ...
        menu::add_item('parent' => $item, 'label' => 'Create a Sub-page',    'url' => '/' . $REQUEST{'url'} . '/?a=create&amp;parent=' . $REQUEST{'current_url'}->{'id'}) if $PERMISSIONS{'content_create'} and $exclude ne 'create';
