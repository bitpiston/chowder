	<xsl:template match="/oyster/forums[@action = 'create_post']" mode="title">New Reply</xsl:template>
	<xsl:template match="/oyster/forums[@action = 'create_post']" mode="heading"><xsl:value-of select="/oyster/forums//thread/@title" /></xsl:template>
	<xsl:template match="/oyster/forums[@action = 'create_post']" mode="description" xml:space="preserve">
	...
	<xsl:template match="/oyster/forums[@action = 'create_post']" mode="content">
		...
		<xsl:if test="/oyster/forums//confirmation">
			...
			<p>If you have not been redirected to your new reply please <a href="{/oyster/@base}forums/post/{/oyster/forums//thread/post/@id}/">click here to go there now</a>.</p><br />
		</xsl:if>
		<xsl:if test="not(/oyster/forums//confirmation)">
			<form class="compose_container" id="posteditor_form" action="{/oyster/@url}{/oyster/@query_string}" method="post">
					...
					<div class="preview">
						...
						<div class="all_posts_container">
							<div class="post_container">
								<div class="post">
									<h2 class="number">#<xsl:value-of select="/oyster/forums//thread/@replies + 2" /></h2>
				...
				<div class="subject">
					<label for="post_subject">Subject: <small>(optional)</small></label>
